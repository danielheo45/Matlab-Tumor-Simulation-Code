...
for j = 1:length(period)
    for k = 1:length(St)
        if mod(St(k), 24*period(j)) >= 0 && mod(St(k), 24*period(j)) <= 24
            injS(j, k) = uS_total/ceil(30/period(j));
        else
            injS(j, k) = 0;
        end
    end
    ...
    [t_pre, y_pre] = ode45(@TANs, tspan, [N1_0, N2_0, T_0, G_0, L_0, S_0],[],St,injS(j, :),Lt,injL(j, :));
    t(j, :, :) = t_pre(:, :);
    y(j, :, :) = y_pre(:, :);
end
toc;
...
plot(t(14,:)./24,injS(14,:),'b',t(6,:)./24,injS(6,:),'r','linewidth',3)
xlabel('Time(days)','fontsize',35)
ylabel('injection rate(u_S)','fontsize',35)
legend('(i)','(ii)')
...
set(0,'DefaultTextFontSize', 25)
set(0,'defaultaxeslinewidth',2)
plot(t(14,:)./24,y(14,:,1),'b',t(14,:)./24,y(14,:,2),'b--',t(6,:)./24,y(6,:,1),'r',t(6,:)./24,y(6,:,2),'r--','linewidth',3)
xlabel('Time(days)','fontsize',35)
ylabel('N2,N1 TANs','fontsize',35)
legend('N1 TANs(i)','N2 TANs(i)','N1 TANs(ii)','N2 TANs(ii)')
...
set(0,'DefaultTextFontSize', 25)
set(0,'defaultaxeslinewidth',2)
plot(t(14,:)./24,y(14,:,3),'b',t(6,:)./24,y(6,:,3),'r','linewidth',3)
xlabel('Time(days)','fontsize',35)
ylabel('Tumor population','fontsize',35)
legend('(i)','(ii)')
